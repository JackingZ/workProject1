...
import { getAllChannelData, getSaleStat, userExportstat } from '@/api/number'
...
export default {
  ...
  async created() {
    ...
    this.channelList = channelList
    this.formData.sourceCode = channelList[0].code
    this.queryAll()
  },
  methods: {
    onTimeChange() {
    ...
    queryAll() {
      ...
      const options = {
        ...this.getDateParams(),
        sourceCode: this.formData.sourceCode ? this.formData.sourceCode : undefined,
    ...
    exportRecord() {
      ...
      userExportstat(options)
        .then(res => {
          ...
          a.href = url
          a.download = '业务员业绩业绩-' + dayjs().format('YYYY-MM-DD HH_mm_ss') + '.xls'
          a.click()
